...
class ArticleType extends FullSEOType
{
   ...
   /**
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        if($is_super_admin) {
            $list_blog = $repoBlogueuse->getBlogueuseforselect();

            $blogeuses = array();
            if(sizeof($list_blog)>0) {
                foreach($list_blog as $key=>$value) {
                    if($value) {
                        $res_blog = array();
                        foreach($value as $blog_id) {
                            $blog =  $repoBlogueuse->findOneById($blog_id);
                            $res_blog[$blog->getPrenom().' '.$blog->getNom()]=$blog;
                        }
                    }
                    $blogeuses[$key] = $res_blog;

                }
            }
        }  else {
            $list_blog = $repoBlogueuse->createQueryBuilder("q")
            ...
            if(sizeof($list_blog)>0) {
                foreach($list_blog as $blog_id) {
                    $blog =  $repoBlogueuse->findOneById($blog_id);
                    $blogeuses[$blog->getPrenom().' '.$blog->getNom()]=$blog;
                }
            }
        }
        ...
        $builder
            ...
            ->add('coupdecoeur', CheckboxType::class, array(
                'label'    => 'Coup de cœur',
                'required' => false,
            ))
            ...
            ->add('lainee', CheckboxType::class, array(
                'label'    => 'lettres des aînées',
                'required' => false,
            ))
            ->add('lblog', CheckboxType::class, array(
                'label'    => 'lettres des blogueuses',
                'required' => false,
            ))
