        ...
        if (in_array("ROLE_ADMIN", $roles)) {
            ...
            if(sizeof($ecoles)>0) {
                foreach($ecoles as $ecole)  {
                    ...
                    $ecole->blogueuses = $blogueuses;
                    $eco_articles[] = $ecole;
                }

            }
            return $this->render('@ViwebBlogueuse/Blogueuse/Admin/blogueuses_ecoles.html.twig', array('ecoles' => $eco_articles));
     ...
     public function deleteAction(Request $request, $blogueuse_id)
          {
            ...
            if ($request->isMethod('POST') && $form->handleRequest($request)->isValid()) {
                $rep = $em->getRepository('ViwebArticleBundle:Article');
